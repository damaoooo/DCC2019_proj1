...
#配置交换机表
switchtable = {
    '127.0.0.1:12100':'127.0.0.1:13101',
     '127.0.0.1:13200':'127.0.0.1:13102',
     '127.0.0.1:14100':'127.0.0.1:13103'
     }

#初始化路由表
Atable = []
routetable.addTable(('127.0.0.1',12000),("127.0.0.1",12100),1,Atable)
routetable.addTable(('127.0.0.1',14000),("127.0.0.1",14100),1,Atable)

a1 = DCC.Unit()
#第一个是本地，后两个乱绑定，不会动用它里面的发送函数
local = ('127.0.0.1',13100)
...
while(1):
    ...
    for sks in rlist:
        ...
        #判断是否是路由信息
        if(src == local):
            print('-'*10,'recv Ctable exchange','-'*10)
